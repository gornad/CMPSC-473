...
int context_switch_number = 0;   /* Number of context switches */
...
void Dispatch(int sig)
{
    ...
    clock_gettime(CLOCK_MONOTONIC, &begin_switch); // Get time before sigsetjump (context switch)
    ...
    start_timer();
    clock_gettime(CLOCK_MONOTONIC, &end_switch); // Get time reight before end of context switch
    context_switch_number++; // Increase the context switch counter
    printf("The number of context switches so far is: %d \n", context_switch_number);
    printf("The time taken by the context switch is: %lld \n", (long long)end_switch.tv_nsec - (long long)begin_switch.tv_nsec); // Print length of context switch
